...
CP-F2.4 实际落地记录（2026-07-27）：

- 扩展现有批次页为原始数据、字段映射、错误清单、字段可用性四视图；默认选中最新批次，并组合当前映射、错误清单与可用性响应展示成功/失败计数、映射版本和三级字段状态。
- 表头映射只复用后端按精确 `header_signature` 返回的不可变版本；当前已应用版本优先，否则自动选择最新版本，不做相似列名或 LLM 猜测。configurator 可编辑并保存新版本，auditor 只能复用并解析，viewer 不请求映射接口且仅查看结果。
- 保存和解析 mutation 均按批次失效 TanStack Query 缓存；解析完成后刷新批次详情、映射、错误清单、字段可用性和批次列表。错误清单保留原始行证据并支持 50 行分页。
- 批次页定向测试 6 passed，覆盖三角色权限、映射保存、解析触发、错误展示和缓存刷新；前端全量 14 passed，strict TypeScript、oxlint、Prettier、生产构建通过，并用受控 mock API 在 1440×1000 无头 Chrome 完成桌面视觉验证。

CP-F2.5 实际落地记录（2026-07-27）：

- 后端全量测试 142 passed、1 skipped；Ruff lint、Ruff 格式检查及 strict mypy（65 个源文件）全部通过。
- OpenAPI 重新导出并生成前端类型客户端，两个受版本控制的契约生成物均无漂移。
- 前端全量测试 14 passed；strict TypeScript、oxlint 零告警、Prettier 格式检查及生产构建全部通过。
- CP-F2.5 门禁时 `expenseguard_test` 位于 `0003 (head)` 且 `alembic check` 无漂移；进入 CP-F3.0 前，默认开发库已完成完整/schema/定向数据三重备份并安全升级至 `0003`，行数、外键、受保护唯一约束和审计触发器复核通过。
...
- [x] **CP-F2.4：** 现有桌面批次页四视图工作流；三角色、映射保存、解析触发、错误展示、缓存刷新与浏览器视觉验证通过。
- [x] **CP-F2.5：** 全量测试、契约同步、交付门禁和状态文件更新；后端 142 passed、1 skipped，前端 14 passed，其余门禁全部通过。
