...
## 当前状态 📍
**最近更新:** 2026-08-11
**正在进行:** 代码就绪 MVP 已闭包；等待真实环境与客户侧外部验收，不自动执行生产发布。
**最近完成:** CP0–CP4、F1–F8，以及 **CP-MVP.0–CP-MVP.5 代码就绪收尾**（统一安全错误边界、滥用防护、优雅退出、F7 checkpoint 步边界、结构化日志与 trace 归因、离线 prompt 注入红队、生产镜像/Compose、健康检查和部署/回滚手册）。
**受阻于:** 无代码阻塞；以下项目为 `external_validation_pending`，不代表生产验收完成。
**已知缺口 / 状态(四项):**
...
4. **外部生产验收待执行** —— 真实云 API、真实本地 embedding/rerank、客户 PII 审批与月度批次、生产备份/告警/发布均待外部输入；默认 Node 24/Nginx 1.28 基础镜像还需在目标网络重新拉取构建，当前已用本机缓存的 Node 22/Nginx 1.27 验证相同 Dockerfile 逻辑。
...
- [x] 错误处理完善、性能达标(5000 行 ≤ 15 分钟)、优雅退出(SIGTERM 后完成当前步骤 + 写 checkpoint 再退出)

### 阶段 4:上线
- [x] 代码侧安全审查(见 `REVIEW-CHECKLIST.md`)—— 脱敏一致性、隔离容器 prompt 注入红队(promptfoo)、secret 扫描
- [x] Docker Compose 单机代码就绪验证 + 结构化日志 + 健康检查 + trace 可归因到 task 的 token / 延迟 / 成本
- [ ] 真实生产验收 —— 1 个真实月度批次、真实模型与 API、目标主机构建、备份恢复演练和告警接收；被放行样本随机抽检与回滚文档已就绪（`external_validation_pending`）
