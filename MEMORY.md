...
- 2026-08-01 — **F6 CP-F6.2 纯确定性核心闭包完成。** 新增 persistence-agnostic `app.core.detection`：四类 strict/frozen profile、source-row/runtime/evidence 判别联合直接复用 F2 `NormalizedExpenseRecord/UnifiedField`，以 `ensure_ascii=False` 紧凑 canonical UTF-8 + SHA-256 固定 profile/group/finding identity；canonical 对 float/未知对象 fail closed。capability primary reason 用显式全序 lattice，parse error 进入 source 分母，零 finding 不降级，系统异常不伪装 unavailable。四 detector 分别固定贪心非重叠拆单、ASCII 后缀最大链、Fraction median/MAD、exact alias 无向 zone pair；全局按 detector/first row/rows/key 稳定去重排序并对 key/payload 不一致 fail closed。新增无 random/float/hash/regex 的独立跨行 synth pattern builder，未改旧单行随机生成契约。36 项 detection + 4 项 synth 定向测试通过；detection statement/branch 90.90%，canonical/finding-key helper 100%；后端全量 437 passed/1 skipped，Ruff/170 文件 format/strict mypy（121 源文件）、OpenAPI/client 连续二次哈希、pre-commit/gitleaks 与 `uvx pip-audit --strict` 全绿。纯核心运行路径不访问数据库/网络/环境变量/当前时间，未实现 service/API/UI/CP-F6.3+ 或 F7/F8。
- 2026-07-31 — **F6 CP-F6.1 持久化闭包完成。** 只新增 `0008_f6_cross_row_detection.py`，未修改 `0001`–`0007`；新增 immutable `detection_config`/`detection_run`/`detection_request`/`correlation_finding_row`，并把 legacy capability/correlation 空 skeleton 强化为 run 级身份。全部 F6 FK 为 RESTRICT，config/run/user/file/finding/expense-row 由完整复合 FK 闭合；旧 capability file 级 unique 替换为 run 级 unique，旧 correlation row 数组删除并由物理参与行表重建；六类事实均拒绝 UPDATE/DELETE，downgrade 在任一 F6 事实存在时于任何 DDL 前拒绝。config 同时冻结 JSONB 与 canonical JSON 文本，数据库校验两者语义等价并以 canonical UTF-8 字节数执行 256 KiB 上限，精确 canonical 语法留给 CP-F6.2 Pydantic。默认库 pre-0008 备份位于 `data/private/backups/cp-f6.1/pre-0008-20260731-140425/`；affected/full/schema SHA-256 分别为 `a405e3ee6d5f65025a5f2adea7476de7ecd81c74805040ae80fc156973aef60d`、`67ec5b6a95983bde97bb42c0cf63cf46cb1247e1a8a9b3f7a17823533d4f8d5f`、`479da9031b0103df79369a9a0ac0f959068bc1514fbacd28685f7904d662b224`，均通过 `pg_restore --list`、容器/本地 hash 交叉校验及 full archive 隔离恢复。F6+基础迁移定向 `24 passed`，迁移/恢复组 `44 passed`，后端全量 `397 passed, 1 skipped`；Ruff/160 文件 format/strict mypy（117 源文件）、默认/测试双库 `0008` Alembic 零漂移、pre-commit/gitleaks 与 `pip-audit --strict` 全绿。未实现 detector/service/API/UI 或 F7/F8。
...
- CP-F6.3 实测：新增服务集成 **13 passed**，F3/F5/F6 定向回归 **90 passed**；detection 49 项 statement/branch **89%**；后端全量 **450 passed, 1 skipped**。Ruff、178 文件 format、strict mypy（127 个源文件）、默认/测试双库 `0008 (head)` 与 Alembic 零漂移通过；OpenAPI/client 无语义漂移，pre-commit/gitleaks、`uvx pip-audit --strict` 全绿。
- CP-F6.2 实测：detection **36 passed** + F6 synth **4 passed**；detection statement/branch **90.90%**，canonical/finding-key helper **100%**；后端全量 **437 passed, 1 skipped**；Ruff、170 文件 format、strict mypy（121 个源文件）通过。OpenAPI/client 连续二次 SHA-256 稳定且无 API 漂移；pre-commit/gitleaks、`uvx pip-audit --strict` 全绿。
- CP-F5.5 实测：F5 服务/API 定向 **19 passed**，迁移/抽样/复核/恢复定向 **56 passed**；后端全量 **392 passed, 1 skipped**；Ruff lint、157 文件 format、strict mypy（115 个源文件）、默认/测试双库 `0007 (head)` 与 Alembic 零漂移通过。OpenAPI/client 连续二次 SHA-256 稳定；前端 **10 个文件、45 passed**、typecheck/oxlint/Prettier/生产 build 通过；pre-commit/gitleaks 与两端依赖审计零漏洞。5000 行 F1→F5 **115.895928 秒**，queue/detail/decision p95 均 <2 秒；Chrome 15 份 metrics/25 张 1440×1000 截图全绿。
...
- [x] F6 CP-F6.0–CP-F6.4（规格、持久化、强类型 profile/capability、四类纯 detector、原子 run/query、request ledger、幂等与 hard-kill 恢复、API/OpenAPI 与桌面补充工作流）
